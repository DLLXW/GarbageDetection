...
class PaddingSqure(object):
    ...
    def __call__(self, image, boxes, labels):
        ...
        ratio = random.uniform(1, 4)
        if height > width:
            ratio_raw = height / width

            left = random.uniform(0, width * ratio * ratio_raw - width)
            top = random.uniform(0, height * ratio - height)

            expand_image = np.zeros(
                (int(height * ratio), int(width * ratio * ratio_raw), depth),
                dtype=image.dtype)
        else:
            ratio_raw = width / height

            left = random.uniform(0, width * ratio - width)
            top = random.uniform(0, height * ratio * ratio_raw - height)

            expand_image = np.zeros(
                (int(height * ratio * ratio_raw), int(width * ratio), depth),
                dtype=image.dtype)
        expand_image[:, :, :] = self.mean
        expand_image[int(top):int(top + height),
        int(left):int(left + width)] = image
