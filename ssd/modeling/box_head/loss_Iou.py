...
import sys,os
#from .box_utils import  bbox_overlaps_iou, bbox_overlaps_giou, bbox_overlaps_diou, bbox_overlaps_ciou
...
def convert_locations_to_boxes(locations, priors, center_variance,size_variance):
    ...
    boxes = torch.cat((
        ...
        priors[:, 2:] * torch.exp(locations[:, 2:] * size_variance)), 1)
    boxes[:, :2] -= boxes[:, 2:] / 2
    boxes[:, 2:] += boxes[:, :2]
    # print(boxes)
    return boxes
...
class IouLoss(nn.Module):
    ...
    def __init__(self, size_sum=True, losstype='Giou'):
        super(IouLoss, self).__init__()
        self.size_sum = size_sum
        self.loss = losstype


    def forward(self, loc_p, loc_t,priors):
        #priors = PriorBox()().to(loc_p.device)
        loc_p=convert_locations_to_boxes(loc_p, priors, 0.1,0.2)
        loc_t = convert_locations_to_boxes(loc_t, priors, 0.1, 0.2)
        num = loc_p.shape[0]
        decoded_boxes = loc_p
        if self.loss == 'Iou':
...
#gious = IouLoss(size_sum=True, variances=[0.1, 0.2], losstype='Ciou')
#loc_p=torch.randn(2, 45180, 4)
#loc_t=torch.randn(464,4)
#loss_l = gious(loc_p, loc_t)
#print(loss_l)
